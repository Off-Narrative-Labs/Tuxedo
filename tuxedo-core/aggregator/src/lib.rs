    ...
    let output = quote! {
        ...
        impl tuxedo_core::constraint_checker::Accumulator for #accumulator_type {
            ...
            fn key_path(intermediate: Self::ValueType) -> & 'static str {
                match intermediate {
                    #(
                        Self::ValueType::#variants3(inner) => <<#inner_types3 as tuxedo_core::ConstraintChecker<#verifier>>::Accumulator as tuxedo_core::constraint_checker::Accumulator>::key_path(inner),
                    )*
                }
            }
            ...
            fn accumulate(acc: Self::ValueType, next: Self::ValueType) -> Result<Self::ValueType, ()> {
                match acc {
                    #(
                        Self::ValueType::#variants4(_) => <<#inner_types4 as tuxedo_core::ConstraintChecker<#verifier>>::Accumulator as tuxedo_core::constraint_checker::Accumulator>::accumulate(acc.0, next.0),
                    )*
                }
            }
        }
        ...
        impl tuxedo_core::ConstraintChecker<#verifier> for #outer_type {
            ...
            fn check (
                ...
                match self {
                    #(
                        Self::#variants5(inner) => inner.check(inputs, peeks, outputs)
                        .map(|old| {
                        ...
                        .map_err(|e| Self::Error::#variants5(e)),
                    )*
                }
            }
        }
