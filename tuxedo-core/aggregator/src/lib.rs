...
pub fn tuxedo_constraint_checker(attrs: TokenStream, body: TokenStream) -> TokenStream {
    ...
    let output = quote! {
        ...
        #vis enum #accumulator_value_type {
            #(
                #variants2(<#inner_types2 as tuxedo_core::ConstraintChecker<#verifier>>::Accumulator),
            )*
        }
        ...
        impl tuxedo_core::constraint_checker::Accumulator for #accumulator_type {
            ...
            fn key_path(intermediate: Self::ValueType) -> & 'static str {
                match intermediate {
                    #(
                        Self::ValueType::#variants3(inner) => <#inner_types3 as tuxedo_core::constraint_checker::Accumulator>::key_path(inner),
                    )*
                }
            }
            ...
            fn accumulate(acc: Self::ValueType, next: Self::ValueType) -> Result<Self::ValueType, ()> {
                match a {
                    #(
                        Self::ValueType::#variants4(_) => <#inner_types4 as tuxedo_core::constraint_checker::Accumulator>::accumulate(a.0, b.0),
                    )*
                }
            }
        }
