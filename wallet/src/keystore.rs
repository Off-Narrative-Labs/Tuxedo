...
pub fn sign_with(keystore: &LocalKeystore, public: &Public, message: &[u8]) -> anyhow::Result<Vec<u8>> {
    keystore
...
pub fn insert_key(keystore: &LocalKeystore, seed: &str) -> anyhow::Result<()> {
    ...
    keystore
        .insert_unknown(KEY_TYPE, &seed, public_key.as_ref()).map_err(|()| anyhow!("Error inserting key"))?;
    Ok(())
}
...
pub fn generate_key(keystore: &LocalKeystore, password: Option<String> ) -> anyhow::Result<()> {
    ...
    keystore
        .insert_unknown(KEY_TYPE, phrase.as_ref(), pair.public().as_ref()).map_err(|()| anyhow!("Error inserting key"))?;
    Ok(())
}
